...
import com.iamadedo.phoneapp.BluetoothService
import com.iamadedo.phoneapp.DeviceInfoData
import com.iamadedo.phoneapp.FileTransferData
...
class WatchFaceActivity : AppCompatActivity(), BluetoothService.ServiceCallback {
    ...
    fun requestWatchFileList(path: String) {
        bluetoothService?.sendMessage(com.iamadedo.phoneapp.ProtocolHelper.createRequestFileList(path))
    }

    fun sendBluetoothMessage(msg: com.iamadedo.phoneapp.ProtocolMessage) {
        bluetoothService?.sendMessage(msg)
    }

    fun deleteWatchFaceOnWatch(path: String) {
        val msg = com.iamadedo.phoneapp.ProtocolHelper.createDeleteFile(path)
        bluetoothService?.sendMessage(msg)
    ...
    fun renameWatchFile(oldPath: String, newName: String) {
        ...
        val msg = com.iamadedo.phoneapp.ProtocolHelper.createRenameFile(oldPath, newPath)
        bluetoothService?.sendMessage(msg)
        tabLayout.postDelayed({ managerFragment.refresh() }, 1000)
    }

    fun createFolder(path: String) {
        val msg = com.iamadedo.phoneapp.ProtocolHelper.createCreateFolder(path)
        bluetoothService?.sendMessage(msg)
        tabLayout.postDelayed({ managerFragment.refresh() }, 1000)
    }
    ...
    private fun processNextPreview() {
        ...
        if (path != null) {
            ...
            bluetoothService?.sendMessage(com.iamadedo.phoneapp.ProtocolHelper.createRequestPreview(path!!))
    ...
    fun applyWatchFace(relPath: String, fileUri: Uri?) {
        ...
        val msg = com.iamadedo.phoneapp.ProtocolHelper.createSetWatchFace(relPath)
        bluetoothService?.sendMessage(msg)
