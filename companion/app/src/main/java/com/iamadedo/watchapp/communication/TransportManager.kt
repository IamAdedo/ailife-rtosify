...
class TransportManager(
    ...
    fun getConnectionStatusString(): String {
        ...
        if (!btConnected && !bleConnected && !wifiConnected && !internetConnected) {
            return context.getString(com.iamadedo.watchapp.R.string.status_disconnected)
        }

        val types = mutableListOf<String>()
        if (btConnected) types.add(context.getString(com.iamadedo.watchapp.R.string.transport_bt))
        if (bleConnected) types.add(context.getString(com.iamadedo.watchapp.R.string.transport_ble))
        if (wifiConnected) types.add(context.getString(com.iamadedo.watchapp.R.string.transport_lan))
        if (internetConnected) types.add(context.getString(com.iamadedo.watchapp.R.string.transport_internet))

        return context.getString(com.iamadedo.watchapp.R.string.status_connected_via, types.joinToString("+"))
    }
