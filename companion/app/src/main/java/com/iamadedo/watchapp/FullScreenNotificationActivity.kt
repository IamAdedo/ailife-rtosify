...
import com.iamadedo.watchapp.ProtocolHelper
import com.iamadedo.watchapp.BluetoothService
import com.iamadedo.watchapp.NotificationData
import com.iamadedo.watchapp.FileDetectedData
...
class FullScreenNotificationActivity : AppCompatActivity() {
    ...
    private fun fileToNotification(data: FileDetectedData): NotificationData {
        ...
        return NotificationData(
            packageName = "com.iamadedo.phoneapp.file",
            title = data.name,
            text = textValue,
            key = "file_${data.path}",
    ...
    private fun displayCurrentNotification() {
        if (currentIndex in notifications.indices) {
            ...
            if (data.packageName == "com.iamadedo.phoneapp.file") {
    ...
    private fun popNotification(dismissOnPhone: Boolean) {
        if (currentIndex in notifications.indices) {
            val data = notifications[currentIndex]
            if (dismissOnPhone && data.packageName != "com.iamadedo.phoneapp.file") {
