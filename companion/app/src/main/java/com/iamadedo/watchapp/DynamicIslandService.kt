...
import com.iamadedo.watchapp.MessageType
...
class DynamicIslandService : Service(), LifecycleOwner, ViewModelStoreOwner, SavedStateRegistryOwner {
    ...
    companion object {
        ...
        const val ACTION_UPDATE_BACKGROUND = "com.iamadedo.watchapp.UPDATE_BACKGROUND"
    }
                ...
                override fun onReceive(context: Context?, intent: Intent?) {
                    when (intent?.action) {
                        BluetoothService.ACTION_FILE_DETECTED -> {
                            ...
                            if (dataJson != null) {
                                try {
                                    val data = Gson().fromJson(dataJson, com.iamadedo.watchapp.FileDetectedData::class.java)
    ...
    private fun createOverlayView() {
        ...
        overlayView.onCallAction = { action ->
            ...
            val intent = Intent("com.iamadedo.watchapp.CALL_ACTION")
            intent.setPackage(packageName)
        ...
        overlayView.onAlarmAction = { action ->
            ...
            val intent = Intent("com.iamadedo.watchapp.ALARM_ACTION")
            intent.setPackage(packageName)
            intent.putExtra("action", action)
            sendBroadcast(intent)
        ...
        overlayView.onMediaAction = { action, position ->
            ...
            if (action == "seek") {
                ...
                val intent = Intent("com.iamadedo.watchapp.MEDIA_ACTION")
                intent.setPackage(packageName)
                intent.putExtra("action", action)
                sendBroadcast(intent)
            }
    ...
    private fun registerReceivers() {
        val filter =
                IntentFilter().apply {
                    ...
                    addAction("com.iamadedo.watchapp.INCOMING_CALL")
                    addAction("com.iamadedo.watchapp.CALL_STATE_CHANGED")
                    addAction(MediaControlActivity.ACTION_MEDIA_STATE_UPDATE)
                    addAction("com.iamadedo.watchapp.ALARM_TRIGGERED")
                    addAction("com.iamadedo.watchapp.ALARM_TRIGGERED")
                    addAction("com.iamadedo.watchapp.ALARM_CLEARED")
    ...
    private val fileDataCache = mutableMapOf<String, com.iamadedo.watchapp.FileDetectedData>()

    private fun showFileNotification(data: com.iamadedo.watchapp.FileDetectedData) {
